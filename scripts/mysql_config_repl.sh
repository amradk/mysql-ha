...
SLAVE_HOSTS=(172.31.1.102 172.31.1.103)
...
mysqldump -h $MASTER_HOST "-u$USER" "-p$PASS" --set-gtid-purged=OFF --opt $DB > $DUMP_FILE
echo "  - Dump complete."
...
do
	...
	cat $DUMP_FILE | mysql -h $SLAVE_HOST "-u$USER" "-p$PASS" $DB 
